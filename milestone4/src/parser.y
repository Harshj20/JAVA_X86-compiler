    ...
    if (!isDot)
    {
        ...
        if(!symTables[t1].entries[lex][0].isfunction &&  !class_to_symboltable[lex]){
            if(!symTables[t1].name.empty()){
                    $$->threeACCode.push_back("\tt" + to_string(tcounter) + " = [%rbp+16] + " + to_string(symTables[t1].entries[lex][0].offset));
...
QualifiedName : Name s_dot Identifier
{
    ...
    {
        string s($3);
        int t1 = symTables[$1->symid].grand_lookup(s);
        $1->symid = t1;
        if (!t1)
        {
...
ClassDeclaration : NormalClassDeclaration
{
    ...
    if(!isDot){
        ...
        if(className != "System" && className != "PrintStream"){
            int t1 = class_to_symboltable[className];
            auto a = symTables[t1].entries.find(className);
            if(a == symTables[t1].entries.end() || a->second.size() > 1){
                ...
                for(auto i = symTables[t1].entries.begin(); i != symTables[t1].entries.end(); i++){
                    if(i->second.size() > 1 && !i->second[0].isfunction){
                        ...
                        $$->threeACCode.push_back("\tt" + to_string(tcounter++) + " = allocate " + to_string(temp));
                        $$->threeACCode.push_back("\tt" + to_string(tcounter) + " = [%rbp+16] + " + to_string(i->second[0].offset));
                        $$->threeACCode.push_back("\t(t" + to_string(tcounter) + ") = t" + to_string(tcounter-1));
                        tcounter++;
                    }
                }
...
MethodDeclarator : Identifier S_open_paren FormalParameterList s_close_paren
{
    ...
    if (!isDot)
    {
        ...
        if(symTables[symTables[currentSymTableId].parentID].name == entryClass && returnFunctionName == "main"){
            ...
            $$->threeACCode.push_back("\tcall " + className + ".ctor");
            $$->threeACCode.push_back("\t%rsp = %rsp + 8");
            $$->threeACCode.push_back("[%rbp+16] = %rax");
            $$->threeACCode.push_back("#MakingFunctionCall");
...
BasicForStatement : k_for invoke_paren s_semicolon s_semicolon s_close_paren Statement
{
    ...
    {
        $$->threeACCode.push_back("L" + to_string(currentSymTableId) + ":");
        $$->threeACCode.push_back("\tt" + to_string(tcounter) + " = 1");
        $$->threeACCode.push_back("\tif t" + to_string(tcounter++) +  " goto L" + to_string(lcounter));
        $$->threeACCode.push_back("\tgoto L" + to_string(loopscope.back()));
        $$->threeACCode.push_back("L" + to_string(lcounter) + ":");
        $$->threeACCode.insert($$->threeACCode.end(), $6->threeACCode.begin(), $6->threeACCode.end());
        $6->threeACCode.clear();
...
| false_Literal
{
    ...
    if(!isDot){
        $$->threeACCode.push_back("\tt" + to_string(tcounter) + " = 0");
        $$->field = "t" + to_string(tcounter++);
    }
}
| null_Literal
...
| Name S_open_paren_c ArgumentList s_close_paren
{
    ...
    if (!isDot)
    {
        ...
        $$->threeACCode.push_back("\tcall " + symTables[$1->symid].name + "." + $1->id);
        $$->threeACCode.push_back("\t%rsp = %rsp + " + to_string($3->sz + offsetVal[OBJECT]));
        if($$->type != VOID){
            $$->threeACCode.push_back("\t" + $$->field + " = %rax");
        }
        $3->threeACCode.clear();
...
| Primary s_dot Identifier S_open_paren_c ArgumentList s_close_paren
{
    ...
    if (!isDot)
    {
        ...
        $$->threeACCode.push_back("\tcall " + $1->id + "." + s);
        $$->threeACCode.push_back("\t%rsp = %rsp + " + to_string($5->sz + offsetVal[OBJECT]));
        if ($$->type != VOID){
            $$->threeACCode.push_back("\t" + $$->field + " = %rax");
        }
        isAssign = false;
